                ...
                if ($check_user_booth === true) {
                    ...
                    if ($request->has('event_id') && $request->event_id == '6') {
                        ...
                        if (!empty($selected_slot)) {
                            ...
                            $success['assembly_name'] = $user_booth->assembly->asmb_name ?? '';
                            $success['booth_name'] = $user_booth->booth_name;
                            $success['total_voters'] = $user_booth->tot_voters ?? '';
                            $success['last_vote_polled'] = $last_vote_polled ?? '';
                            $success['last_vote_polled_time'] = $date_time_received ?? '';
                            $success['votes_polled_till'] = $selected_slot ?? 'Slot not available';
                    ...
                    $success['state_name'] = $electionInfo->electionState->name ?? '';
                    $success['district_name'] = $electionInfo->electionDistrict->name ?? '';
                    $success['assemble_name'] = $electionInfo->electionAssembly->asmb_name ?? '';
                    $success['booth_name'] = $electionInfo->electionBooth->booth_name ?? '';
                    $success['event_name'] = $electionInfo->electionEvent->event_name ?? '';
                    $success['event_status'] = $electionInfo->status ?? '';
                    $success['ac_type'] = $electionInfo->electionAssembly->ac_type ?? '';
                    $success['st_code'] = $electionInfo->electionState->st_code ?? '';
                    $success['asmb_code'] = $electionInfo->electionAssembly->asmb_code ?? '';
